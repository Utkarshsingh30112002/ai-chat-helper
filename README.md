...
- Calls a **Node.js** backend for **three** AI reply suggestions (OpenAI-compatible API).
...
1. Copy `backend/.env.example` → `backend/.env` and set `APP_SECRET`, `OPENAI_API_KEY`, and optional `OPENAI_BASE_URL` / `MODEL`.
